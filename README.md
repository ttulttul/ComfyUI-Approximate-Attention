...
When `comet_experiment` is not provided, `Flux2TTRRuntime` now auto-generates a per-runtime Comet experiment key using `git rev-parse --short=7 HEAD` plus a start timestamp (`YYYYMMDD-HHMMSS`), and enables persistent experiment reuse for that key. Comet display names are also set at experiment start to a human-readable run tag of the form `YYYY-MM-DD-HHMMSS-<git6>` (for example `2026-02-11-073025-b3ed63`) to make run identification easier in the Comet UI.
