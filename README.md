...
- Local window can be scheduled by sigma via `local_window_min/max` and `local_window_sigma_low/high` (window shrinks as sigma decreases; use `local_window` when schedule is disabled). A value of `0` means "full attention", so scheduled endpoints use the current sequence length as the full window.
